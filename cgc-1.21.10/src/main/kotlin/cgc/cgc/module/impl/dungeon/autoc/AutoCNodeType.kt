...
enum class AutoCNodeType(
	...
	ETHERWARP("etherwarp", "/ac add etherwarp [exactlyPos]", EtherwarpNode::supply),
	MOVING_ETHERWARP("movingetherwarp", "/ac add movingetherwarp", MovingEtherwarpNode::supply),
	...
	WAIT("wait", "/ac add wait <seconds>", WaitNode::supply),
	TRACK("track", "/ac add track <seconds> <x> <y> <z>", TrackNode::supply),
	...
	BREAK("break", "/ac add break <true|false> <seconds> [notMoving]", BreakNode::supply);

	fun supply(player: LocalPlayer, args: String): AutoCNode? =
		factory(player, args)

	fun isEtherwarp(): Boolean =
		this == ETHERWARP || this == MOVING_ETHERWARP
